...
export class MinaHTTPClient {
  ...
  async textToSpeech(deviceId: string, text: string): Promise<boolean> {
    // 不同型号/固件支持的 TTS ubus 方法不一致：
    //  - text_to_speech / mibrain   ← 多数设备通用（xiaogpt 等验证）
    //  - player_play_tts / mediaplayer ← 部分设备
    const attempts: Array<[string, string]> = [
      ['text_to_speech', 'mibrain'],
      ['player_play_tts', 'mediaplayer'],
    ];
    for (const [method, path] of attempts) {
      const r = await this.ubusRequestRaw(deviceId, method, path, { text });
      if (r === null) {
        // 网络/鉴权层面失败，换下一种方法
        host.log.info(`[TTS] ${method}/${path} -> HTTP fail`);
        continue;
      }
      if (r.code === 0) {
        host.log.info(`[TTS] ${method}/${path} -> OK`);
        return true;
      }
      // 关键：小米云对设备侧 ubus 转发常返回 code=101（“invalid result”），
      // 但设备实际已收到并完成播报。这是已知的误报，按“已下发”处理，
      // 不再继续尝试第二种方法（否则会重复下发、可能造成重复播报）。
      if (r.code === 101) {
        host.log.info(`[TTS] ${method}/${path} -> code=101（设备通常已播报），按已下发处理`);
        return true;
      }
      // 其它非 0 码才算真失败，记录并尝试下一种方法
      host.log.warn(`[ubus] ${method}/${path} code=${r.code} msg=${(r as any).message || ''}`);
    }
    return false;
  }
  ...
  async getLatestAskFromXiaoai(deviceId: string, hardware: string, limit = 2): Promise<AskMessage[]> {
    host.log.info(`[ConversationMonitor] getLatestAskFromXiaoai deviceId=${deviceId} hardware=${hardware} limit=${limit} useMinaForAsk=${shouldUseMinaForAsk(hardware)}`);
    // 部分设备需要通过 ubus 方式获取
    if (shouldUseMinaForAsk(hardware)) {
      const ubusResult = await this.getLatestAskByUbus(deviceId);
      host.log.info(`[ConversationMonitor] getLatestAskByUbus result: ${ubusResult ? ubusResult.length : 0} messages`);
      return ubusResult;
    }
    ...
    host.log.info(`[ConversationMonitor] getLatestAskFromXiaoai apiUrl=${apiUrl}`);

    // 大多数设备通过 xiaoai API 获取，带3次重试
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      const messages = await this.doGetLatestAskFromXiaoai(deviceId, apiUrl);
      if (messages !== null) {
        host.log.info(`[ConversationMonitor] getLatestAskFromXiaoai attempt=${attempt} success, ${messages.length} messages`);
        return messages;
      }
      host.log.info(`[ConversationMonitor] getLatestAskFromXiaoai attempt=${attempt} returned null, retrying...`);
    }
    host.log.info(`[ConversationMonitor] getLatestAskFromXiaoai all ${MAX_RETRIES} attempts failed`);
    return [];
  }
  ...
  /**
   * 执行 UBus 请求（原始版）：只要 HTTP 层成功就返回云端解析后的响应体，
   * 不对 code 做判断（code 可能非 0）。仅当网络/鉴权失败时返回 null。
   */
  async ubusRequestRaw(deviceId: string, method: string, path: string, message: Record<string, unknown>): Promise<UbusResponse | null> {
    ...
    return this.doPostRequest<UbusResponse>(apiUrl, body);
  }

  /**
   * 执行 UBus 请求（严格版）：code !== 0 视为失败返回 null。
   */
  async ubusRequest(deviceId: string, method: string, path: string, message: Record<string, unknown>): Promise<UbusResponse | null> {
    const result = await this.ubusRequestRaw(deviceId, method, path, message);
    ...
    // 检查响应码
    if (result.code !== 0) {
      host.log.warn(`[ubus] ${method}/${path} code=${result.code} msg=${(result as any).message || ''}`);
      return null;
    }
  ...
  private async doGetLatestAskFromXiaoai(deviceId: string, apiUrl: string): Promise<AskMessage[] | null> {
    ...
    try {
      ...
      host.log.info(`[ConversationMonitor] doGetLatestAskFromXiaoai fetch error: ${String(e)}`);
      return null;
    }

    host.log.info(`[ConversationMonitor] doGetLatestAskFromXiaoai status=${response.status}`);

    if (response.status === 401) {
      host.log.info(`[ConversationMonitor] doGetLatestAskFromXiaoai 401 token expired`);
      if (this.onTokenExpired) {
    ...
    if (response.status !== 200) {
      host.log.info(`[ConversationMonitor] doGetLatestAskFromXiaoai unexpected status=${response.status}`);
      return null;
    }
    ...
    try {
      const text = response.text() as string;
      // 打印原始响应体（最多 1000 字符）
      host.log.info(`[ConversationMonitor] doGetLatestAskFromXiaoai raw response (${text.length} chars): ${text.substring(0, 1000)}`);
      ...
      if (!dataStr) {
        host.log.info(`[ConversationMonitor] doGetLatestAskFromXiaoai data field is empty/null`);
        return [];
      }

      const dataObj = JSON.parse(dataStr) as ConversationData;
      if (!dataObj.records || dataObj.records.length === 0) {
        host.log.info(`[ConversationMonitor] doGetLatestAskFromXiaoai records empty or missing`);
        return [];
      }
      ...
      host.log.info(`[ConversationMonitor] doGetLatestAskFromXiaoai parsed ${messages.length} messages`);
      return messages;
    } catch (e) {
      host.log.info(`[ConversationMonitor] doGetLatestAskFromXiaoai parse error: ${String(e)}`);
      return null;
    }
  }
