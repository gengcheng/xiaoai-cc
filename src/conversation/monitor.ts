...
export class ConversationMonitor {
  ...
  start(): void {
    ...
    if (this.enabled && this.pollTimer !== null) {
      host.log.info('[ConversationMonitor] Already running, skip start');
      return;
    }
    ...
    this.refreshDevices().then(() => {
      ...
      host.log.info(`[ConversationMonitor] Started, devices=${this.devices.size} callbacks=${this.callbacks.size}`);
    }).catch(e => {
      host.log.error('[ConversationMonitor] refreshDevices error: ' + String(e));
    });

    // 启动定时轮询
    this.pollTimer = setInterval(() => {
      this.pollAll().catch(e => {
        host.log.error('[ConversationMonitor] pollAll error: ' + String(e));
      });
    }, this.pollInterval);
  }
  ...
  stop(): void {
    ...
    host.log.info(`[ConversationMonitor] Stopped`);
  }
  ...
  registerCallback(name: string, cb: ConversationCallback): void {
    ...
    host.log.info(`[ConversationMonitor] Callback registered: ${name}`);
  }
  ...
  unregisterCallback(name: string): void {
    ...
    host.log.info(`[ConversationMonitor] Callback unregistered: ${name}`);
  }
  ...
  getMessages(limit: number = 50, sinceTimestampMs: number = 0): ConversationMessage[] {
    ...
    host.log.info(`[ConversationMonitor] getMessages total_stored=${this.messages.length} returning=${result.length} (limit=${limit} sinceTs=${sinceTimestampMs})`);
    return result;
  }
  ...
  private async refreshDevices(): Promise<void> {
    ...
    for (const key of this.devices.keys()) {
      if (!managedKeys.has(key)) {
        ...
        host.log.info(`[ConversationMonitor] Device removed from monitoring: ${key}`);
      }
    }
    ...
    for (const dev of newDevices) {
      ...
      host.log.info(`[ConversationMonitor] Device added to monitoring: ${dev.deviceName} (${key})`);
    }
  }
  ...
  private async pollDevice(dm: DeviceMonitorState): Promise<void> {
    ...
    try {
      ...
      host.log.warn(`[ConversationMonitor] Failed to get conversations: ${dm.deviceId} ${String(e)}`);
      return;
    }
    ...
    if (msgCount > 0) {
      ...
      host.log.info(`[ConversationMonitor] pollDevice device=${dm.deviceId} returned ${msgCount} messages: ${summary}`);
    } else {
      host.log.info(`[ConversationMonitor] pollDevice device=${dm.deviceId} returned 0 messages`);
    }
    ...
    // 打印过滤结果
    host.log.info(`[ConversationMonitor] pollDevice device=${dm.deviceId} after filter: ${newMessages.length} new (lastTimestampMs=${dm.lastTimestampMs})`);
    ...
    for (const msg of newMessages) {
      ...
      host.log.info(`[ConversationMonitor] addMessage ts=${msg.message.timestamp_ms} q="${q.substring(0, 80)}" a="${a.substring(0, 80)}"`);
      this.addMessage(msg);
    }

    host.log.info(`[ConversationMonitor] New messages account=${dm.accountId} device=${dm.deviceId} count=${newMessages.length}`);
  ...
  private async notifyCallbacks(messages: ConversationMessage[]): Promise<void> {
    for (const [name, cb] of this.callbacks.entries()) {
      try {
        ...
        host.log.error(`[ConversationMonitor] Callback error name=${name}: ${String(e)}`);
      }
    }
  }
  ...
  private async sendWebhook(wh: WebhookConfig, payload: string): Promise<void> {
    try {
      ...
      host.log.info(`[ConversationMonitor] Webhook sent id=${wh.id} url=${wh.url}`);
    } catch (e) {
      host.log.warn(`[ConversationMonitor] Webhook failed id=${wh.id} url=${wh.url}: ${String(e)}`);
    }
  }
