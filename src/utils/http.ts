...
class ResponseHeaders {
  ...
  getSetCookie(): string[] {
    if (this._setCookies.length > 0) return this._setCookies;
...
export async function httpFetch(
  ...
  // 原插件用自定义头 X-Fetch-No-Redirect 让 Go 宿主不自动跟随 3xx；
  // Node 原生 fetch 改用标准的 redirect:'manual'（undici 会返回真实 3xx + 头）。
  let redirect: 'follow' | 'manual' = 'follow';
  if (headers['X-Fetch-No-Redirect'] || headers['x-fetch-no-redirect']) {
    redirect = 'manual';
    delete headers['X-Fetch-No-Redirect'];
    delete headers['x-fetch-no-redirect'];
  }

  const resp = await fetch(url, { method, headers, body, redirect });

  // Node 的 Response.headers 是 Headers 对象：用 forEach 抽成普通对象，
  // 并单独保留 Set-Cookie 数组（可能有多条）。
...
export async function callHostAPI<T = unknown>(method: string, path: string, body?: unknown): Promise<T> {
  ...
  const pluginToken = await host.plugin.getToken();
  if (!pluginToken) {
    throw new Error('Plugin token not available from host.plugin.getToken()');
  }
